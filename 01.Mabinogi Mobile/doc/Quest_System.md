...
런타임에서는 CQuest가 현재 활성화된 Objective만 대상으로 이벤트를 전달합니다.
...
이벤트 일치 여부·진행도 반영·완료 판정을 파생 클래스가 각각 구현하도록 설계했습니다.
...
이 예시는 Objective가 단순 데이터 보관이 아니라,
자신의 조건과 진행 상태를 스스로 해석하는 단위라는 점을 잘 보여줍니다.
